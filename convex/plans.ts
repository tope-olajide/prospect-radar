...
export const save = internalMutation({
  args: { missionId: v.id("missions"), normalizedGoal: v.string(), mode, mustHave: v.array(v.string()), niceToHave: v.array(v.string()), exclusions: v.array(v.string()), missingFacts: v.array(v.string()), recommendedSources: v.array(v.string()), proposedSteps: v.array(v.string()), completionPredicate: v.string(), model: v.string() },
  ...
  handler: async (ctx, args) => {
    ...
    const value = { ...args, provider: "openai" as const, createdAt: Date.now() };
